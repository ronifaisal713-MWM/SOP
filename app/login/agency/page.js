...
export default function AgencyLoginPage() {
  ...
  async function handleLogin(e) {
    ...
    if (categoryForRole(profile?.role) !== "agency") {
      await supabase.auth.signOut();
      setLoading(false);
      setMessage("This sign-in is for agency owners/admins. Please use the Staff or Client sign-in instead.");
      return;
    }
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-slate-50 px-6">
      ...
      >
        <h1 className="text-xl font-semibold text-brand mb-1">Agency Sign In</h1>
        <p className="text-xs text-slate-400 mb-6">For agency owners and admins.</p>

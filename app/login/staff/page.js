...
export default function StaffLoginPage() {
  ...
  async function handleLogin(e) {
    ...
    if (categoryForRole(profile?.role) !== "staff") {
      await supabase.auth.signOut();
      setLoading(false);
      setMessage("This sign-in is for staff accounts. Please use the Agency or Client sign-in instead.");
      return;
    }
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-slate-50 px-6">
      ...
      >
        <h1 className="text-xl font-semibold text-brand mb-1">Staff Sign In</h1>
        <p className="text-xs text-slate-400 mb-6">For team members of an agency.</p>
